...
const Navigation = () => {
  ...
  return (
    <div className={`relative w-full ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} transition-colors duration-300`}>
      ...
      <div className='max-w-7xl mx-auto px-2 py-10 flex items-center justify-between relative'>
        ...
        <ul className='hidden xl:flex items-center gap-3 uppercase font-semibold text-xs'>
          ...
          <li className='flex items-center gap-1'>
            <p><NavLink to={'recension'} className={({ isActive }) => isActive ? 'text-emerald-600' : 'hover:text-emerald-600 transition-all'}>Recension</NavLink></p>
        ...
        <div className='flex items-center gap-5 text-xl relative'>
          ...
          <div className='relative'>
            ...
            {notificationsVisible && (
              <div className={`absolute z-20 top-full right-0 mt-2 w-64 bg-white border rounded-md shadow-lg ${darkMode ? 'bg-gray-800 text-white border-gray-700' : 'bg-white text-gray-900 border-gray-300'}`}>
                <div className='p-4'>
      ...
      {mobileNavLinks && (
        <div className='fixed inset-0 z-20 flex'>
          ...
          ></div>
          <div className={`bg-white absolute top-0 right-0 w-3/4 md:w-1/2 lg:w-1/3 h-full p-5 shadow-lg transform transition-transform duration-300 ease-in-out translate-x-0 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
